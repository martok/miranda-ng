...
void NetlibSslFree(SslHandle *ssl)
{
	...
	/* Delete Context */
	g_OpenSSL.SSL_free (ssl->session);
	g_OpenSSL.SSL_CTX_free (ssl->ctx);
	ssl->session = NULL;
	ssl->ctx = NULL;
...
static bool ClientConnect(SslHandle *ssl, const char *host)
{
	...
	if (suite != NULL)
		NetlibLogf(NULL, "SSL established with %s", suite);
	return true;
}
	...
	if (err <= 0) {
		...
		switch (err2) {
			case SSL_ERROR_ZERO_RETURN:
				...
				break;
			default:
				NetlibLogf(NULL, "SSL failure recieving data (%d, %d, %d)", err, err2, WSAGetLastError());
				ssl->state = sockError;
				return SOCKET_ERROR;
		}
		return 0;
	...
	if (err <= 0) {
		...
		switch (err2) {
			case SSL_ERROR_ZERO_RETURN:
				...
				break;
			default:
				NetlibLogf(NULL, "SSL failure sending data (%d, %d, %d)", err, err2, WSAGetLastError());
				ssl->state = sockError;
				return SOCKET_ERROR;
		}
		return 0;
